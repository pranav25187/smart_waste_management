...
const createEwaste = async (req, res) => {
  ...
  try {
    ...
    await db.query(
      "INSERT INTO Materials (material_type, location, availability, `condition`, price_range, posted_by) VALUES (?, ?, ?, ?, ?, ?)",
      [type, "Default Location", true, condition, price.toString(), user_id] // Ensure user_id is passed here
    );
